...
void initializeQuiz(Quiz *quiz) {
    QuizQuestion initial_questions[NUM_QUESTIONS] = {
        {"Preorder traversal visits nodes in which order?", 
         {"Left → Root → Right", 
          "Root → Left → Right", 
          "Left → Right → Root", 
          "Right → Root → Left"}, 
        ...
        {"Postorder traversal is typically used for:", 
         {"Searching", 
          "Deleting trees", 
          "Balancing", 
          "Inserting nodes"}, 
        ...
        {"Which traversal would output: D → B → E → A → F → C → G?", 
         {"Preorder", 
          "Inorder", 
          "Postorder", 
          "Level-order"}, 
         1, 1,
         "Root (A) is in the middle, matching inorder."},

        {"To copy a tree, which traversal is most efficient?", 
         {"Preorder", 
          "Inorder", 
          "Postorder", 
          "Level-order"}, 
         0, 1,
         "Preorder preserves hierarchy during copying."}
    };
...
void runQuiz(Quiz *quiz) {
    ...
    printf("\n=== QUIZ COMPLETE ===\n");
    printf("=== Thank you for testing your tree traversals knowledge! ===\n\n");
}
...
int main() {
    ...
    printf("Welcome to the Tree Traversals Quiz!\n");
    printf("This quiz covers preorder, inorder, and postorder traversals.\n");
